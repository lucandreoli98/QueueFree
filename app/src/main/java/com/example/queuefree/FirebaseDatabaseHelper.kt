                    ...
                    for (firms in snapshot.children) {
                        if (firm.id == firms.key) { // se c'è l'id dell'azienda
                            for (users in firms.children) {
                                ...
                                if (id == userBook.substring(0, 28) && yy == userBook.substring(
                                        29,
                                        33
                                    ).toLong() &&
                                    mm == userBook.substring(33, 35)
                                        .toLong() && dd == userBook.substring(35, 37).toLong()
                                ) {
                    ...
                    for (firms in p0.children) {
                        ...
                        for (field in firms.children) {
                            when (field.key) {
                                ...
                                "nomeazienza" -> f[firms.key!!]!!.nomeazienza =
                                ...
                                "descrizione" -> f[firms.key!!]!!.descrizione =
                                    field.value as String
                                "maxTurn" -> f[firms.key!!]!!.maxTurn = field.value as Long
                                "maxPartecipants" -> f[firms.key!!]!!.maxPartecipants =
                                    field.value as Long
                            ...
                            if (idRead[0].trim() == id) { // in questo punto hai la prenotazione nell'azienda
                                ...
                                for (field in book.children) {
                                    when (field.key) {
                                        ...
                                        "nore" -> booking[booking.size - 1].nOre =
                                            field.value as Long
                                        "npartecipanti" -> booking[booking.size - 1].nPartecipanti =
                                            field.value as Long
                                    }
                                }
                            }
                            ...
                            for (userBooks in firms.children) {
                                ...
                                for (field in userBooks.children) {
                                    when (field.key) {
                                        ...
                                        "nore" -> bookings[bookings.size - 1].nOre =
                                            field.value as Long
                                        "npartecipanti" -> bookings[bookings.size - 1].nPartecipanti =
                                            field.value as Long
                                    }
                                }
                    ...
                    for (azienda in snapshot.children) {
                        if (azienda.key == firm.id) {
                            for (prenotazione in azienda.children) {
                                val idRead = prenotazione.key!!.split("-")
                                if (idRead[0].trim() == id) {
                                    var actualbooking = Booking()
                                    for (campi in prenotazione.children) {
                                        when (campi.key) {
                                            "dd" -> actualbooking.dd = campi.value as Long
                                            "mm" -> actualbooking.mm = campi.value as Long
                                            "yy" -> actualbooking.yy = campi.value as Long
                                            "nore" -> actualbooking.nOre = campi.value as Long
                                            "npartecipanti" -> actualbooking.nPartecipanti =
                                                campi.value as Long
                                        }
                                    }
                                    for (i in 0 until bookings.size) {
                                        if (actualbooking == bookings[i]) {
                                            database.getReference("bookings/${firm.id}/$id-${bookings[i].yy}${isZero(bookings[i].mm.toInt())}${bookings[i].dd}-${isZero(bookings[i].nOre.toInt())}").removeValue()
                                        }
